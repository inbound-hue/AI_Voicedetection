...
def get_gmail():
    ...
    # If there are no credentials or they are invalid, re-authenticate
    if not creds or not creds.valid:
        print("No valid credentials found, re-authenticating...")  # Debug print
        flow = InstalledAppFlow.from_client_secrets_file("credentials.json", SCOPES)
        
        # Use run_console() for non-interactive authentication in cloud environments like Render
        creds = flow.run_console()  # Changed from run_local_server to run_console for headless environments
...
def process_emails(target_email):
    ...
    for m in msgs:
        ...
        print(f"Checking message with subject: {msg.get('Subject', '')}")  # Debug print (subject)

        # Skip the email if the subject does not contain the target email address
        ...
        for p in msg.walk():
            if p.get_filename() and p.get_filename().lower().endswith((".mp3", ".wav", ".m4a", ".mp4")):
                ...
                # Save the transcript to HubSpot
                save_transcript_to_hubspot(target_email, transcript, audio_path, data)

                # If appointment date is found, send an email
                if data["appointment_date"]:
                    send_email(target_email, data["appointment_date"])

# If run directly, trigger the email processing
